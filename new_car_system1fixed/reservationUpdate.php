		...
		if(is_numeric($id))
		{
			...
			if(is_numeric($cssn))
			{
				if(strlen($cssn) == 11)
				{
					if(is_numeric($vid))
					{
										$query = "UPDATE reservation SET `SSN_cust`='$cssn', `VehicleID`='$vid', `StartDate`='$sdate', `FinishDate`='$fdate', `DateOfReserv`='$rdate', `Paid`='$paid', `StartLocation`='$sloc', `FinishLocation`='$floc', `LicensePlate`='$lplate' WHERE `ReservationID`='$id'";
...
<div class="container">
<form method="post" action = "reservationUpdate.php">	
<?php echo $error;?>
<div class="form-group row">
        <label class="col-md-2 control-label">Reservation ID</label>
        <div class="col-md-5">
            <input type="text" class="form-control" name="id" placeholder="Reservation ID" value="<?php echo $id;?>" disabled />
</div>
</div>
<div class="form-group row">
